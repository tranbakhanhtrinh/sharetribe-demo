...
import { REVIEW_TYPE_OF_PROVIDER, REVIEW_TYPE_OF_CUSTOMER, propTypes, REVIEW_TYPE_OF_HOBBIES } from '../../util/types';
...
import { HobbiesOfReviewsPage } from '../../components/Reviews/Reviews';
...
export const MobileReviews = props => {
  const { reviews, queryReviewsError, hobbies } = props;
  const reviewsOfHobbies = reviews.filter(r => r.attributes.type === REVIEW_TYPE_OF_HOBBIES);
  ...
  return (
    <div className={css.mobileReviews}>
      <H4 as="h2" className={css.mobileReviewsTitle}>
        <FormattedMessage
          id="ProfilePage.reviewsHobbiesTitle"
          values={{ count: hobbies?.length }}
        />
      </H4>
      <ReviewsErrorMaybe queryReviewsError={queryReviewsError} />
      <HobbiesOfReviewsPage hobbies={hobbies} />
...
export const DesktopReviews = props => {
  const [showReviewsType, setShowReviewsType] = useState(REVIEW_TYPE_OF_HOBBIES);
  const { reviews, queryReviewsError, hobbies } = props;
  const reviewsOfProvider = reviews.filter(r => r.attributes.type === REVIEW_TYPE_OF_PROVIDER);
  const reviewsOfCustomer = reviews.filter(r => r.attributes.type === REVIEW_TYPE_OF_CUSTOMER);
  const isReviewTypeHobbiesSelected = showReviewsType === REVIEW_TYPE_OF_HOBBIES;
  ...
  const desktopReviewTabs = [
    {
      ...
      selected: isReviewTypeHobbiesSelected,
      onClick: () => setShowReviewsType(REVIEW_TYPE_OF_HOBBIES),
    },
    {
      text: (
        <Heading as="h3" rootClassName={css.desktopReviewsTitle}>
...
export const MainContent = props => {
  ...
  return (
    <div>
      ...
      {isMobileLayout ? (
        <MobileReviews reviews={reviews} hobbies={hobbies} queryReviewsError={queryReviewsError} />
      ) : (
        <DesktopReviews reviews={reviews} hobbies={hobbies} queryReviewsError={queryReviewsError} />
      )}
    </div>
  );
};
...
const ProfilePageComponent = props => {
  ...
  return (
    ...
    >
      ...
      >
        <MainContent bio={bio} displayName={displayName} hobbies={publicData?.hobbies} userShowError={userShowError} {...rest} />
      </LayoutSideNavigation>
    </Page>
  );
};
